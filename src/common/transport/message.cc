...
namespace transport {
  ...
  Message::Serialize(uint32_t *length_ptr) {
    ...
    uint32_t length1 = 0;
    unsigned char *data1 = relMessageId_.Serialize(&length1);
    if (length1 <= 127) {
    ...
    uint32_t length2 = 0;
    unsigned char *data2 = label_.Serialize(&length2);
    if (length2 <= 127) {
    ...
    uint32_t length3 = 0;
    unsigned char *data3 = relMessageLabel_.Serialize(&length3);
    if (length3 <= 127) {
  ...
  Message::Deserialize(unsigned char *data, uint32_t length) {
    ...
    relMessageId_.Deserialize(data + c, elength);
    c += elength;
    if ((data[c] & 0x80) == 0) {
      elength = data[c];
      c++;
    ...
    relMessageLabel_.Deserialize(data + c, elength);
    c += elength;
    if ((data[c] & 0x80) == 0) {
      elength = data[c];
      c++;
