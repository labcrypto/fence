 ...
 * Name: response.cc
 * Description:
 *   This file contains implementation of Response class.
 ******************************************************************/
...
namespace gate {
  unsigned char *
  Response::Serialize(uint32_t *length_ptr) {
    ...
    uint32_t length1 = 0;
    unsigned char *data1 = requestId_.Serialize(&length1);
    if (length1 <= 127) {
    ...
    uint32_t length2 = 0;
    unsigned char *data2 = requestLabel_.Serialize(&length2);
    if (length2 <= 127) {
  ...
  void
  Response::Deserialize(unsigned char *data, uint32_t length) {
    ...
    requestId_.Deserialize(data + c, elength);
    c += elength;
    if ((data[c] & 0x80) == 0) {
      elength = data[c];
      c++;
    ...
    requestLabel_.Deserialize(data + c, elength);
    c += elength;
    if ((data[c] & 0x80) == 0) {
      elength = data[c];
      c++;
